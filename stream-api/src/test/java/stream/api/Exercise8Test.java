...
public class Exercise8Test extends ClassicOnlineStore {
    ...
    public void itemsNotOnSale() {
        ...
        Set<String> itemSetNotOnSale = customerStream.flatMap(customer -> customer.getWantToBuy().stream().map(Item::getName).filter(it -> !itemListOnSale.contains(it))).collect(Collectors.toSet());
    ...
    public void havingEnoughMoney() {
        ...
        Predicate<Customer> havingEnoughMoney = customer ->
                customer.getBudget() > onSale
                                        .stream()
                                        .distinct()
                                        .filter(it -> customer.getWantToBuy().contains(it))
